...
router.post('/add', async (req, res) => {
  const { society_id, society_name, society_description } = req.body;
  const connection = await pool.getConnection(); // Get a connection from the pool
  try {
    await connection.beginTransaction(); // Start the transaction
    const query = 'INSERT INTO society (society_id, society_name, society_description) VALUES (?, ?, ?)';
    await connection.execute(query, [society_id, society_name, society_description]);
...
router.post('/:id/edit', async (req, res) => {
  const { society_name, society_description } = req.body;
  const society_id = req.params.id;

  try {
    // Correct order of parameters for the UPDATE query
    const query = 'UPDATE society SET society_name = ?, society_description = ? WHERE society_id = ?';
    await pool.execute(query, [society_name, society_description, society_id]); // Move `society_id` to the end
    res.redirect('/admin');
  } catch (error) {

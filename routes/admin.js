...
router.get('/society', ensureAuthenticated, ensureAdmin, async (req, res) => {
  try {
    ...
    // Map societies and find the president for each society
    const societyData = await Promise.all(
      societies.map(async (society) => {
        // Fetch the president from the members table for the given society_id
        const president = await Members.findOne({
          where: {
            society_id: society.dataValues.society_id,
            position: 'President',
          },
        });
        ...
        return {
          ...
          president: president ? president.dataValues.name : 'TBA',
        };
      })
    );
    // Render the page with society data
